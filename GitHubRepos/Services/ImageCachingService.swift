...
protocol ImageCachingServiceType {
    func load(from imageURL: URL?, completion: ((UIImage?, Bool) -> Void)?)
    func cleanAllCache()
}
...
class ImageCachingService: ImageCachingServiceType {
    ...
    func load(from imageURL: URL?, completion: ((UIImage?, Bool) -> Void)? = nil) {
        //print("ImageCachingService start url \(imageURL?.absoluteString ?? "")")
        guard let url = imageURL?.encodedURL, let absoluteString = url.encodedURL?.absoluteString else {
            completion?(nil, false)
            return
        }

        //check for cache
        if let image = loadImageFromCache(url: absoluteString) {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            let session = URLSession.shared.dataTask(with: request, completionHandler: { [weak self] data, response, _ in
                if let data = data, let response = response, ((response as? HTTPURLResponse)?.statusCode ?? 500) < 300, let image = UIImage(data: data) {
                    ...
                    if image.size.width > screenWidth, let resized = image.resizedTo(width: screenWidth/2.0), let resizedData = resized.jpegData(compressionQuality: 0.8) { // resize image
